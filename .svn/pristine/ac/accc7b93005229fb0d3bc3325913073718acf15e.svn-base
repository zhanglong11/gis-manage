...
class Main extends Component {
    ...
    renderTreeNodes = data =>
        data.map(item => {
            if (item.children) {
                return (
                    <TreeNode icon={<Icon type={item.isLayer?'api':'apartment'} />} title={item.name} key={item.key} dataRef={item}>
                        {this.renderTreeNodes(item.children)}
                    </TreeNode>
                );
            }
            return <TreeNode icon={<Icon type={item.isLayer?'api':'apartment'} />} {...item} title={item.name} dataRef={item}/>;
        });
        ...
        return (
            <Layout style={{height: '100%'}}>
                <Sider theme='light'>
                    ...
                    {treeData.length ?
                        <Tree showIcon onSelect={this.selectTree} onRightClick={this.modify} defaultExpandAll>
